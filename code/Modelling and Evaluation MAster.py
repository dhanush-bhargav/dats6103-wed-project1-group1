...
knn_cm3 = confusion_matrix(y_test, knn_model3.predict(X_test))
conf_matrices['KNN (k=250)'] = knn_cm3
plot_confusion_matrix(knn_cm3, ['Non-Fraud', 'Fraud'])
...
rf_cm3 = confusion_matrix(y_test, rf_model3.predict(X_test))
conf_matrices['Random Forest (50, 20)'] = rf_cm3
plot_confusion_matrix(rf_cm3, ['Non-Fraud', 'Fraud'])

rf_cm4 = confusion_matrix(y_test, rf_model4.predict(X_test))
conf_matrices['Random Forest (50, 10)'] = rf_cm4
plot_confusion_matrix(rf_cm4, ['Non-Fraud', 'Fraud'])
...
# %%
rf_probs3 = rf_model3.predict_proba(X_test)[:, 1]
fpr_rf3, tpr_rf3, _ = roc_curve(y_test, rf_probs3)
auc_rf3 = roc_auc_score(y_test, rf_probs3)

plt.figure(figsize=(8, 6))
plt.plot(fpr_rf3, tpr_rf3, label='Random Forest (50, 20) (AUC = %0.2f)' % auc_rf3)
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.title('ROC Curve - Random Forest (50, 20)')
plt.legend()
plt.show()
# %%
rf_probs4 = rf_model4.predict_proba(X_test)[:, 1]
fpr_rf4, tpr_rf4, _ = roc_curve(y_test, rf_probs4)
auc_rf4 = roc_auc_score(y_test, rf_probs4)

plt.figure(figsize=(8, 6))
plt.plot(fpr_rf4, tpr_rf4, label='Random Forest (50, 10) (AUC = %0.2f)' % auc_rf4)
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.title('ROC Curve - Random Forest (50, 10)')
plt.legend()
plt.show()
# %%
...
# Assuming you've trained an SVC model named 'svc_model' and have X_test, y_test available

# Calculate the predicted probabilities for the positive class
y_proba_svc = svc_model.decision_function(X_test)
...
# Plot the ROC curve
plt.figure(figsize=(8, 6))
plt.plot(fpr_svc, tpr_svc, color='blue', lw=2, label=f'SVC ROC curve (AUC = {roc_auc_svc:.2f})')
